...
def postprocess(sidang, user_id=DEFAULT_USER_ID, penguji=DEFAULT_PENGUJI, ta=DEFAULT_TA, pembimbing=DEFAULT_PEMBIMBING, revisi_terakhir=DEFAULT_REVISI_TERAKHIR, mhs=DEFAULT_MHS, pembimbing_dosen=DEFAULT_PEMBIMBING_DOSEN, **kwargs):
    if ta:
        ta = sidang.ta
    if penguji:
    ...
    if user_id and revisi_terakhir:
        me = [p for p in penguji if p.id == user_id]
        me = me[0] if me else None
        if me:
            revisi_terakhir = sorted(me.revisi, key=lambda x: x.nomor)[-1]
            revisi_terakhir = models.Revisi.postprocess(
                revisi_terakhir, sidang=False)
        else:
...
def get(sidang_id, user_id=DEFAULT_USER_ID, penguji=DEFAULT_PENGUJI, ta=DEFAULT_TA, pembimbing=DEFAULT_PEMBIMBING, revisi_terakhir=DEFAULT_REVISI_TERAKHIR, mhs=DEFAULT_MHS, pembimbing_dosen=DEFAULT_PEMBIMBING_DOSEN, **kwargs):
...
def fetch(user_id=DEFAULT_USER_ID, penguji=DEFAULT_PENGUJI, ta=DEFAULT_TA, pembimbing=DEFAULT_PEMBIMBING, revisi_terakhir=DEFAULT_REVISI_TERAKHIR, mhs=DEFAULT_MHS, pembimbing_dosen=DEFAULT_PEMBIMBING_DOSEN, status=None, ta_status=None, ta_type=None, **kwargs):
    if ta_status:
